...
$targets = @(
    ...
    [pscustomobject]@{ Plugin = "TRBarrels"; Check = "FoodInstance aging-level member (obfuscated canary: GALGEBMEJLA, prop-or-field)"; Found = ((Test-MemberByName -TypeName "FoodInstance" -MemberName "GALGEBMEJLA" -MemberKind "Property") -or (Test-MemberByName -TypeName "FoodInstance" -MemberName "GALGEBMEJLA" -MemberKind "Field")) },
    [pscustomobject]@{ Plugin = "TRAutoloader"; Check = "FoodInstance cached-price field (obfuscated canary: JOKIOEMCNKP)"; Found = Test-MemberByName -TypeName "FoodInstance" -MemberName "JOKIOEMCNKP" -MemberKind "Field" },
