...
$targets = @(
    ...
    [pscustomobject]@{ Plugin = "TRBarrels"; Check = "FoodInstance aging-level member (obfuscated canary: GKAFDFGINHI, prop-or-field)"; Found = ((Test-MemberByName -TypeName "FoodInstance" -MemberName "GKAFDFGINHI" -MemberKind "Property") -or (Test-MemberByName -TypeName "FoodInstance" -MemberName "GKAFDFGINHI" -MemberKind "Field")) },
    [pscustomobject]@{ Plugin = "TRAutoloader"; Check = "FoodInstance cached-price field (obfuscated canary: KFDLGPGAALJ)"; Found = Test-MemberByName -TypeName "FoodInstance" -MemberName "KFDLGPGAALJ" -MemberKind "Field" },
